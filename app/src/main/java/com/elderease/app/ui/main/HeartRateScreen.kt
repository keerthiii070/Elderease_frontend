...
// Check finger presence first
        if (time - lastFingerCheckTime > 500) {
            lastFingerCheckTime = time
            val present = avg < 210
            onFingerPresent(present)
        }

// ✅ Only send samples if finger is present
        val fingerDetected = avg < 210
        if (fingerDetected) {
            onSample(time, avg)
        }
...
private fun HistoryTabContent(
    ...
    val cardDateFormatter = remember { SimpleDateFormat("MMM dd, hh:mm a", Locale.getDefault()) }
    val selectedDateFormatter = remember { SimpleDateFormat("MMM dd, yyyy", Locale.getDefault()) }
    ...
    Column(
        ...
        val firstDayOfMonth = (currentCalendar.clone() as Calendar).apply { set(Calendar.DAY_OF_MONTH, 1) }
            ...
            Row(
                ...
                for (col in 0 until 7) {
                    ...
                    if (dayNum < 1 || dayNum > daysInMonth) {
                        ...
                        val dateCal = (currentCalendar.clone() as Calendar).apply { set(Calendar.DAY_OF_MONTH, dayNum) }
                        val isSelected = dateCal.get(Calendar.YEAR) == selectedCalendar.get(Calendar.YEAR) &&
                                dateCal.get(Calendar.DAY_OF_YEAR) == selectedCalendar.get(Calendar.DAY_OF_YEAR)
                        ...
                        val isToday = dateCal.get(Calendar.YEAR) == todayCalendar.get(Calendar.YEAR) &&
                                dateCal.get(Calendar.DAY_OF_YEAR) == todayCalendar.get(Calendar.DAY_OF_YEAR)
                            ...
                            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                ...
                                if (hasRecord) {
                                    Box(
                                        modifier = Modifier
                                            .size(6.dp)
                                            .background(if (isSelected) Color.White else Color(0xFF6A5DF6), CircleShape)
                                    )
                                } else if (isToday) {
                                    Box(
                                        modifier = Modifier
                                            .size(6.dp)
                                            .background(if (isSelected) Color.White else Color.LightGray, CircleShape)
                                    )
                                }
                            }
                        }
        ...
        DisposableEffect(Unit) {
            val analyzer = HeartRateAnalyzer(
                onSample = { t, v ->
                    ...
                    progress += 0.005f   // ✅ now sample only comes when finger is present
                },
                onFingerPresent = { present ->
            ...
            if (progress >= 1.0f) {
                ...
                if (bpm != null) {
                    ...
                    if (confidence >= 60) {
                        ...
                        Toast.makeText(context, "Measurement unstable, try again", Toast.LENGTH_SHORT).show()
                    }
                }
                progress = 0f
                samples = emptyList()
                ...
                Column(
                    ...
                    if (finalResultBpm != null && !measuring) {
                        ...
                        val (msg, color) = when (status) {
                            HeartStatus.LOW -> "Your heart rate is lower than average." to Color(0xFFFFA726)
                            HeartStatus.HEALTHY -> "Your heart rate is normal." to Color(0xFF4CAF50)
                            HeartStatus.HIGH -> "Your heart rate is higher than average." to Color(0xFFF44336)
                        }
                        Text(
                            text = msg,
                            color = color,
                        ...
                        Button(
                            ...
                            Text("Measure Again", fontSize = 16.sp, fontWeight = FontWeight.Bold)
                        }
                    } else if (measuring) {
                        Spacer(Modifier.height(10.dp))
                        Box(
